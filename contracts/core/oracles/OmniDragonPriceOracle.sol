...
contract OmniDragonPriceOracle is Ownable, Pausable, ReentrancyGuard {
  ...
  IOmniDragonRegistry public hybridRegistry;
  ...
  constructor(
    ...
    address _hybridRegistry,
    address _dragonToken
  ) Ownable(_initialOwner) {
    nativeSymbol = _nativeSymbol;
    quoteSymbol = _quoteSymbol;
    deploymentTime = block.timestamp;
    hybridRegistry = IOmniDragonRegistry(_hybridRegistry);
  ...
  function _isWrappedNative(address token) internal view returns (bool) {
    if (address(hybridRegistry) == address(0)) return false;

    try hybridRegistry.getWrappedNativeToken(uint16(block.chainid)) returns (address wrappedNative) {
